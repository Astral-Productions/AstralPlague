...
#include "AstralCameraMode_ThirdPerson.h"
#include "AstralPlague/Camera/AstralCameraMode.h"
#include "Components/PrimitiveComponent.h"
#include "Curves/CurveVector.h"
#include "Engine/Canvas.h"
#include "GameFramework/CameraBlockingVolume.h"
#include "AstralCameraAssistInterface.h"
#include "AstralPenetrationAvoidanceFeeler.h"
#include "GameFramework/Controller.h"
#include "GameFramework/Character.h"
#include "Math/RotationMatrix.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(AstralCameraMode_ThirdPerson)

namespace AstralCameraMode_ThirdPerson_Statics
{
...
UAstralCameraMode_ThirdPerson::UAstralCameraMode_ThirdPerson()
{
	...
	PenetrationAvoidanceFeelers.Add(FAstralPenetrationAvoidanceFeeler(FRotator(+00.0f, +00.0f, 0.0f), 1.00f, 1.00f, 14.f, 0));
	PenetrationAvoidanceFeelers.Add(FAstralPenetrationAvoidanceFeeler(FRotator(+00.0f, +16.0f, 0.0f), 0.75f, 0.75f, 00.f, 3));
	PenetrationAvoidanceFeelers.Add(FAstralPenetrationAvoidanceFeeler(FRotator(+00.0f, -16.0f, 0.0f), 0.75f, 0.75f, 00.f, 3));
	PenetrationAvoidanceFeelers.Add(FAstralPenetrationAvoidanceFeeler(FRotator(+00.0f, +32.0f, 0.0f), 0.50f, 0.50f, 00.f, 5));
	PenetrationAvoidanceFeelers.Add(FAstralPenetrationAvoidanceFeeler(FRotator(+00.0f, -32.0f, 0.0f), 0.50f, 0.50f, 00.f, 5));
	PenetrationAvoidanceFeelers.Add(FAstralPenetrationAvoidanceFeeler(FRotator(+20.0f, +00.0f, 0.0f), 1.00f, 1.00f, 00.f, 4));
	PenetrationAvoidanceFeelers.Add(FAstralPenetrationAvoidanceFeeler(FRotator(-20.0f, +00.0f, 0.0f), 0.50f, 0.50f, 00.f, 4));
}

void UAstralCameraMode_ThirdPerson::UpdateView(float DeltaTime)
{
...
void UAstralCameraMode_ThirdPerson::UpdateForTarget(float DeltaTime)
{
...
void UAstralCameraMode_ThirdPerson::DrawDebug(UCanvas* Canvas) const
{
...
void UAstralCameraMode_ThirdPerson::UpdatePreventPenetration(float DeltaTime)
{
	...
	IAstralCameraAssistInterface* TargetControllerAssist = Cast<IAstralCameraAssistInterface>(TargetController);

	IAstralCameraAssistInterface* TargetActorAssist = Cast<IAstralCameraAssistInterface>(TargetActor);
	...
	IAstralCameraAssistInterface* PPActorAssist = OptionalPPTarget.IsSet() ? Cast<IAstralCameraAssistInterface>(PPActor) : nullptr;
	...
	if (PPActorRootComponent)
	{
		...
		IAstralCameraAssistInterface* AssistArray[] = { TargetControllerAssist, TargetActorAssist, PPActorAssist };

		if (AimLineToDesiredPosBlockedPct < ReportPenetrationPercent)
		{
			for (IAstralCameraAssistInterface* Assist : AssistArray)
			{
				if (Assist)
				{
...
void UAstralCameraMode_ThirdPerson::PreventCameraPenetration(class AActor const& ViewTarget, FVector const& SafeLoc, FVector& CameraLoc, float const& DeltaTime, float& DistBlockedPct, bool bSingleRayOnly)
{
#if ENABLE_DRAW_DEBUG
	...
	//TODO IAstralCameraTarget.GetIgnoredActorsForCameraPentration();
	...
	{
		FAstralPenetrationAvoidanceFeeler& Feeler = PenetrationAvoidanceFeelers[RayIdx];
		if (Feeler.FramesUntilNextTrace <= 0)
		{
			...
			if (bHit && HitActor)
			{
				...
				if (HitActor->ActorHasTag(AstralCameraMode_ThirdPerson_Statics::NAME_IgnoreCameraCollision))
				{
					bIgnoreHit = true;
...
void UAstralCameraMode_ThirdPerson::SetTargetCrouchOffset(FVector NewTargetOffset)
{
...
void UAstralCameraMode_ThirdPerson::UpdateCrouchOffset(float DeltaTime)
{
