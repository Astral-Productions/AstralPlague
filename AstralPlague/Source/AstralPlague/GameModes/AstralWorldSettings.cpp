...
#include UE_INLINE_GENERATED_CPP_BY_NAME(AstralWorldSettings)
...
FPrimaryAssetId AAstralWorldSettings::GetDefaultGameplayExperience() const
{
	...
	if (!DefaultGameplayExperience.IsNull())
	{
		...
		if (!Result.IsValid())
		{
			UE_LOG(LogAstralExperience, Error, TEXT("%s.DefaultGameplayExperience is %s but that failed to resolve into an asset ID (you might need to add a path to the Asset Rules in your game feature plugin or project settings"),
...
#if WITH_EDITOR
void AAstralWorldSettings::CheckForErrors()
{
	...
	for (TActorIterator<APlayerStart> PlayerStartIt(GetWorld()); PlayerStartIt; ++PlayerStartIt)
	{
		...
		if (IsValid(PlayerStart) && PlayerStart->GetClass() == APlayerStart::StaticClass())
		{
			MapCheck.Warning()
				...
				->AddToken(FTextToken::Create(FText::FromString("is a normal APlayerStart, replace with AAstralPlayerStart.")));
		}
	}
