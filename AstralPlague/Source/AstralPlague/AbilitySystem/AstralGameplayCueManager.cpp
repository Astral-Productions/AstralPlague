...
#include "AstralGameplayCueManager.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(AstralGameplayCueManager)

//////////////////////////////////////////////////////////////////////

enum class EAstralEditorLoadMode
{
...
namespace AstralGameplayCueManagerCvars
{
	static FAutoConsoleCommand CVarDumpGameplayCues(
		TEXT("Astral.DumpGameplayCues"),
		TEXT("Shows all assets that were loaded via AstralGameplayCueManager and are currently in memory."),
		FConsoleCommandWithArgsDelegate::CreateStatic(UAstralGameplayCueManager::DumpGameplayCues));

	static EAstralEditorLoadMode LoadMode = EAstralEditorLoadMode::LoadUpfront;
}
...
UAstralGameplayCueManager::UAstralGameplayCueManager(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
}

UAstralGameplayCueManager* UAstralGameplayCueManager::Get()
{
	return Cast<UAstralGameplayCueManager>(UAbilitySystemGlobals::Get().GetGameplayCueManager());
}

void UAstralGameplayCueManager::OnCreated()
{
	Super::OnCreated();
...
void UAstralGameplayCueManager::LoadAlwaysLoadedCues()
{
	if (ShouldDelayLoadGameplayCues())
	{
		...
		for (const FName& CueTagName : AdditionalAlwaysLoadedCueTags)
		{
			...
			{
				UE_LOG(LogAstral, Warning, TEXT("UAstralGameplayCueManager::AdditionalAlwaysLoadedCueTags contains invalid tag %s"), *CueTagName.ToString());
			}
		}
	}
}

bool UAstralGameplayCueManager::ShouldAsyncLoadRuntimeObjectLibraries() const
{
	switch (AstralGameplayCueManagerCvars::LoadMode)
	{
	case EAstralEditorLoadMode::LoadUpfront:
		return true;
	case EAstralEditorLoadMode::PreloadAsCuesAreReferenced_GameOnly:
#if WITH_EDITOR
		if (GIsEditor)
		{
			return false;
...
#endif
		break;
	case EAstralEditorLoadMode::PreloadAsCuesAreReferenced:
		break;
	}
...
bool UAstralGameplayCueManager::ShouldSyncLoadMissingGameplayCues() const
{
	return false;
}

bool UAstralGameplayCueManager::ShouldAsyncLoadMissingGameplayCues() const
{
	return true;
}

void UAstralGameplayCueManager::DumpGameplayCues(const TArray<FString>& Args)
{
	UAstralGameplayCueManager* GCM = Cast<UAstralGameplayCueManager>(UAbilitySystemGlobals::Get().GetGameplayCueManager());
	if (!GCM)
	{
		UE_LOG(LogAstral, Error, TEXT("DumpGameplayCues failed. No UAstralGameplayCueManager found."));
		return;
	}

	const bool bIncludeRefs = Args.Contains(TEXT("Refs"));

	UE_LOG(LogAstral, Log, TEXT("=========== Dumping Always Loaded Gameplay Cue Notifies ==========="));
	for (UClass* CueClass : GCM->AlwaysLoadedCues)
	{
		UE_LOG(LogAstral, Log, TEXT("  %s"), *GetPathNameSafe(CueClass));
	}

	UE_LOG(LogAstral, Log, TEXT("=========== Dumping Preloaded Gameplay Cue Notifies ==========="));
	for (UClass* CueClass : GCM->PreloadedCues)
	{
		...
		UE_LOG(LogAstral, Log, TEXT("  %s (%d refs)"), *GetPathNameSafe(CueClass), NumRefs);
		if (bIncludeRefs && ReferencerSet)
		{
			for (const FObjectKey& Ref : *ReferencerSet)
			{
				UObject* RefObject = Ref.ResolveObjectPtr();
				UE_LOG(LogAstral, Log, TEXT("    ^- %s"), *GetPathNameSafe(RefObject));
			}
		}
	}

	UE_LOG(LogAstral, Log, TEXT("=========== Dumping Gameplay Cue Notifies loaded on demand ==========="));
	...
	if (GCM->RuntimeGameplayCueObjectLibrary.CueSet)
	{
		for (const FGameplayCueNotifyData& CueData : GCM->RuntimeGameplayCueObjectLibrary.CueSet->GameplayCueData)
		{
			if (CueData.LoadedGameplayCueClass && !GCM->AlwaysLoadedCues.Contains(CueData.LoadedGameplayCueClass) && !GCM->PreloadedCues.Contains(CueData.LoadedGameplayCueClass))
			{
				...
				UE_LOG(LogAstral, Log, TEXT("  %s"), *CueData.LoadedGameplayCueClass->GetPathName());
			}
		}
	}

	UE_LOG(LogAstral, Log, TEXT("=========== Gameplay Cue Notify summary ==========="));
	UE_LOG(LogAstral, Log, TEXT("  ... %d cues in always loaded list"), GCM->AlwaysLoadedCues.Num());
	UE_LOG(LogAstral, Log, TEXT("  ... %d cues in preloaded list"), GCM->PreloadedCues.Num());
	UE_LOG(LogAstral, Log, TEXT("  ... %d cues loaded on demand"), NumMissingCuesLoaded);
	UE_LOG(LogAstral, Log, TEXT("  ... %d cues in total"), GCM->AlwaysLoadedCues.Num() + GCM->PreloadedCues.Num() + NumMissingCuesLoaded);
}

void UAstralGameplayCueManager::OnGameplayTagLoaded(const FGameplayTag& Tag)
{
		...
		TGraphTask<FGameplayCueTagThreadSynchronizeGraphTask>::CreateTask().ConstructAndDispatchWhenReady([]()
			{
				if (GIsRunning)
				{
					if (UAstralGameplayCueManager* StrongThis = Get())
					{
...
void UAstralGameplayCueManager::HandlePostGarbageCollect()
{
...
void UAstralGameplayCueManager::ProcessLoadedTags()
{
	...
	if (GIsRunning)
	{
		...
		{
			UE_LOG(LogAstral, Warning, TEXT("UAstralGameplayCueManager::OnGameplayTagLoaded processed loaded tag(s) but RuntimeGameplayCueObjectLibrary.CueSet was null. Skipping processing."));
		}
	}
}

void UAstralGameplayCueManager::ProcessTagToPreload(const FGameplayTag& Tag, UObject* OwningObject)
{
	switch (AstralGameplayCueManagerCvars::LoadMode)
	{
	case EAstralEditorLoadMode::LoadUpfront:
...
void UAstralGameplayCueManager::OnPreloadCueComplete(FSoftObjectPath Path, TWeakObjectPtr<UObject> OwningObject, bool bAlwaysLoadedCue)
{
...
void UAstralGameplayCueManager::RegisterPreloadedCue(UClass* LoadedGameplayCueClass, UObject* OwningObject)
{
...
void UAstralGameplayCueManager::HandlePostLoadMap(UWorld* NewWorld)
{
	if (RuntimeGameplayCueObjectLibrary.CueSet)
	{
...
void UAstralGameplayCueManager::UpdateDelayLoadDelegateListeners()
{
...
bool UAstralGameplayCueManager::ShouldDelayLoadGameplayCues() const
{
...
void UAstralGameplayCueManager::RefreshGameplayCuePrimaryAsset()
{
